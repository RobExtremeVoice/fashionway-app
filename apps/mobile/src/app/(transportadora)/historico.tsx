...
export default function TransportadoraHistoricoScreen() {
  ...
  const TABS: { key: Tab; label: string; count: number; color: string }[] = [
    { key: 'ao_vivo',   label: 'Ao vivo',   count: aoVivo.length,    color: '#7C3AED' },
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#F1F5F9' }}>
      ...
      <View style={{
        ...
        <View style={{ position: 'absolute', top: -60, right: -60, width: 180, height: 180, borderRadius: 90, backgroundColor: 'rgba(139,92,246,0.35)' }} />
        <Text style={{ color: '#DDD6FE', fontSize: 13, fontWeight: '700', letterSpacing: 0.5 }}>HISTÓRICO</Text>
        <Text style={{ color: '#fff', fontSize: 26, fontWeight: '900', marginTop: 4 }}>Operações</Text>
        ...
        <View style={{ flexDirection: 'row', gap: 8, marginTop: 18 }}>
          {TABS.map((t) => (
            ...
            >
              ...
              {t.count > 0 && (
                <View style={{ backgroundColor: 'rgba(255,255,255,0.25)', borderRadius: 10, paddingHorizontal: 7, paddingVertical: 1 }}>
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={isLoading} onRefresh={() => { fetchActiveColetas(); fetchPastColetas() }} tintColor="#7C3AED" />
        }
      >
        {currentList.length === 0 ? (
          <View style={{ backgroundColor: '#fff', borderRadius: 20, padding: 40, alignItems: 'center', borderWidth: 1, borderColor: '#E2E8F0', marginTop: 16 }}>
            <Text style={{ fontSize: 48, marginBottom: 12 }}>🚛</Text>
            <Text style={{ fontSize: 16, fontWeight: '800', color: '#111827' }}>
              {tab === 'ao_vivo' ? 'Nenhuma operação ao vivo' : tab === 'pendentes' ? 'Nenhuma pendente' : 'Nenhuma operação concluída'}
            </Text>
          </View>
        ) : (
          currentList.map((c) => {
            ...
            return (
              <TouchableOpacity
                ...
                activeOpacity={0.85}
                style={{ backgroundColor: '#fff', borderRadius: 18, padding: 16, marginBottom: 10, borderWidth: 1, borderColor: '#E2E8F0' }}
              >
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                  ...
                  <View style={{ alignItems: 'flex-end' }}>
                    <Text style={{ fontSize: 16, fontWeight: '900', color: '#111827' }}>{formatBRL((c as any).valorFrete ?? 0)}</Text>
                    <Text style={{ fontSize: 10, color: '#94A3B8', marginTop: 2 }}>{fmtDate((c as any).updatedAt ?? (c as any).createdAt)}</Text>
                  </View>
                </View>
